...
import com.miniplay.custom.ObserverHelpers.YoutubeQueue.StatusMessage;
 ...
 */
public class YoutubeQueueObserver extends AbstractObserver {

    public static String QUEUE_STATUS_URL = "";
    public Gson gson = new Gson();
    public static final String LOG_PREPEND = "> Youtube Queue Observer: ";
    ...
    public void runObserver() {
        ...
        if(queueStatusOutput != null) { // Only if the queue status fetch returned true
            ...
            ElasticBalancer.getInstance().queue_workers.set(((Double)statusMessage.total.get("workforce")).intValue());
            ElasticBalancer.getInstance().workers_queued_jobs.set(((Double)statusMessage.total.get("workload")).intValue());
